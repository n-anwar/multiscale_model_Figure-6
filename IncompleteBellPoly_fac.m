function OutMatrix = IncompleteBellPoly_fac(Nin,Kin,DataList,fac)
 ...
 for (nidx=2:Nin) %row, Note: Row 1 has only 1 non-zero element
  ...
  for (kidx=2:Kin) %column, Note: Column 1 is the given input array
   ...
   for midx=1:nidx-kidx+1 %recursion
    Bm(nidx,kidx) = Bm(nidx,kidx) +fac(nidx)/(fac(nidx-midx+1)*fac(midx))*DataList(midx)*Bm(nidx-midx,kidx-1); %nchoosek(nidx-1,midx-1); factorial(nidx-1)/(factorial(nidx-midx)*factorial(midx-1))
   end
