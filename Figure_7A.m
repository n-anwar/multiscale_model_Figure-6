...
mosquito=linspace(0,3,100); %range for sensitivity analysis

m_sen_I=zeros(1,length(mosquito)); %prealocating space for storing Steady state I value
m_sen_L=zeros(1,length(mosquito)); %prealocating space for storing Steady state L value

%% Running simulation for each value of alpha
for l=1:length(mosquito)
par.m=mosquito(l);   %mosquito per human  
    
    tmax=7*365;      %max simulation time
    step=50000;
...
%% Figure
plot(mosquito,m_sen_I,'linewidth',2.0)
hold on
plot(mosquito,m_sen_L,'--','linewidth',2.0)
xline(.58,'--','HandleVisibility','off')

ax = gca;
ax.FontSize = 15;
xlabel('Mosquito per human, m','fontweight','bold','Fontsize',20)
