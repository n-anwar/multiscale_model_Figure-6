...
hypnozoite=linspace(0,10,50); %range for sensitivity analysis

nu_sen_I=zeros(1,length(hypnozoite)); %prealocating space for storing Steady state I value
nu_sen_L=zeros(1,length(hypnozoite)); %prealocating space for storing Steady state L value

%% Running simulation for each value of alpha
for l=1:length(hypnozoite)
par.nu=hypnozoite(l);  %Number of hypnozoites per bite 
...
%% Figure
plot(hypnozoite,nu_sen_I,'linewidth',2.0)
hold on
plot(hypnozoite,nu_sen_L,'--','linewidth',2.0)
xline(5,'--','HandleVisibility','off')

ax = gca;
ax.FontSize = 15;
xlabel('Average hypnozoite per bite, \nu','fontweight','bold','Fontsize',20)
