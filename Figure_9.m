...
par.mu=1/425;  %hypnozoites death rate
...
Hyp_dist_I_SIL=(Hyp_SS-Hyp_L_SS*NoRel_Inf)/(1-NoRel_Inf);  %prob of i hyp given infection
...
[t_w,y_w]=ode45(@white,[0 tmax],[s_h0,i_h0,s_m0,e_m0,i_m0],options,par,imax);
...
Hyp_dist_white=zeros(imax+1,1); %probability of having hypnozoite from white model
...
hyp_number=0:imax;
bar(hyp_number(1:3:end),Hyp_dist(1:3:end,:),'BarWidth',1);
hold on
plot(hyp_number,Hyp_dist,'-.');
ax = gca;
ax.FontSize = 15;
...
subplot(2,2,3)   
%% finding probability
Hyp_dist_L_white=zeros(imax,1); %probability of having hypnozoite in Liver-stage infected from white model

Hyp_dist_L_SIL=Hyp_L_SS(:)/(1-Prob_nohyp_giv_noInf(end));
hyp_number_H=1:imax;

tot_s_h=sum(s_h(end,2:end),2);  

Hyp_dist_L_white(:)=s_h(end,2:end)/tot_s_h;  %normalising

Hyp_dist_L=[Hyp_dist_L_SIL Hyp_dist_L_white];
bar(hyp_number_H(1:3:end),Hyp_dist_L(1:3:end,:),'BarWidth',1);
hold on
plot(hyp_number_H,Hyp_dist_L,'-.');
ax = gca;
ax.FontSize = 15;
...
subplot(2,2,4)
%% finding probability
Hyp_dist_I_white=zeros(imax,1); %probability of having hypnozoite in blood-stage infected from white model
Hyp_dist_I=zeros(imax+1,2);
% Hyp_dist_I_SIL_incl_0=zeros(imax,1);


tot_i_h=sum(i_h(end,:),2);
% Hyp_dist_I_SIL_incl_0(1)=p(end);
Hyp_dist_I_white(1)=i_h(end,1)/tot_i_h;  %normalising


Hyp_dist_I(1,:)=[p(end) Hyp_dist_I_white(1)];
for i=2:imax+1
Hyp_dist_I_white(i)=i_h(end,i)/tot_i_h;
% Hyp_dist_I_SIL_incl_0(i+1)=Hyp_dist_I_SIL(i);
Hyp_dist_I(i,:)=[Hyp_dist_I_SIL(i-1) Hyp_dist_I_white(i)];
end

% Hyp_dist_I=[Hyp_dist_I_SIL_incl_0 Hyp_dist_I_white];
bar(hyp_number(1:3:end),Hyp_dist_I(1:3:end,:),'BarWidth',1);
% set(d(1),'FaceColor','b');
% set(d(2),'FaceColor','r');
hold on
plot(hyp_number,Hyp_dist_I,'-.');
ax = gca;
ax.FontSize = 15; 
xlabel('Hypnozoites in blood-stage infected individuals','fontweight','bold','Fontsize',20)
ylabel('Probability','fontweight','bold','Fontsize',20)
legend('Our multi-scale model','White et al. model','fontsize',17)
xlim([-1 50])
title('D')
...
function ydot=white(~,y,par,imax)
