...
activation_rate=linspace(0,.02,100); %range for sensitivity analysis

alpha_sen_I=zeros(1,length(activation_rate)); %prealocating space for storing Steady state I value
alpha_sen_L=zeros(1,length(activation_rate)); %prealocating space for storing Steady state L value

%% Running simulation for each value of alpha
for l=1:length(activation_rate)
    par.alpha=activation_rate(l);  %hypnozoites activation rate
...
%% Figure
plot(activation_rate,alpha_sen_I,'linewidth',2.0)
hold on
plot(activation_rate,alpha_sen_L,'--','linewidth',2.0)
xline(1/332,'--','HandleVisibility','off')

ax = gca;
ax.FontSize = 15;
xlabel('Hypnozoite activation rate (1/day), \alpha','fontweight','bold','Fontsize',20)
